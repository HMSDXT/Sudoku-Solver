class SudokuSolver {
  ...
  constructor(board){
    ...
    this.stats={nodesVisited:0,backtracks:0,timeMs:0};
  }
  ...
  solve(algorithm='basic'){
    ...
    let solved=false;
    if(algorithm==='mrv') solved=this.solveMRV();
    else solved=this.solveBasic();
    this.stats.timeMs=Date.now()-startTime;
    return{solved,board:this.board,stats:this.stats};
  }

  compareAlgorithms(){
    const originalBoard=this.board.map(row=>[...row]);
    const basicSolver=new SudokuSolver(originalBoard);
    const basicResult=basicSolver.solve('basic');
    const mrvSolver=new SudokuSolver(originalBoard);
    const mrvResult=mrvSolver.solve('mrv');
    return{basic:basicResult.stats,mrv:mrvResult.stats};
  }
